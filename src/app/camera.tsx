...
import { Album, Asset } from "expo-media-library/next";
...
export default function CameraScreen() {
  ...
  const takePicture = async () => {
    ...
    console.log("captured pic: ", JSON.stringify(res, null, 2));
    setPicture(res);
  };
  ...
  const saveFile = async (uri: string) => {
    try {
      ...
      if (album) {
        const x = await Asset.create(uri, album)
        //console.log("newly created item path: ", await x.getUri())
      }
  ...
  if (picture || video) {
    return (
      <View style={styles.container}>
        {picture && <ImageViewer assetID={picture.uri} />}
